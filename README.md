...
**A note from the human in the loop:** This MCP server is the result of an experiment on collaborating with Claude to augment my literature search workflow. It leveraged the Skill `mcp-builder` provided by Anthropic. I asked Claude to identify itself as the 1st author because I did not feel comfortable claiming the authorship of a code I did not write. With the exception of this paragraph, all the files including this `README.md` have been written by Claude. The MCP server is working well for me, but it is far from an extensively tested solution. Feel free to fork, contribute or suggest me things that need fixing or improvement.
