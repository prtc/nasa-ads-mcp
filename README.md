...
This repository contains a code sample fully generated by Claude (Anthropic), with human collaboration from Paula Coelho.
**License:** MIT
...
**A note from the human in the loop:** This MCP server is the result of an experiment on collaborating with Claude to augment my literature search workflow. It leveraged the Skill `mcp-builder` provided by Anthropic. The MCP server is working well for me, but it is far from an extensively tested solution. Feel free to fork, contribute or suggest me things that need fixing or improvement.
